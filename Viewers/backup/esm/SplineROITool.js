...
class SplineROITool extends ContourSegmentationBaseTool {
    ...
    constructor(toolProps = {}, defaultToolProps = {
        ...
        this._addNewAnnotationFromIndex = (element, idxPos) => {
            ...
            let boundary = idxPos.map(array => utilities.transformIndexToWorld(imageData, array));

            const splineConfig = this._getSplineConfig(SplineTypesEnum.Linear);
        ...
        this._calculateCachedStats = (annotation, element) => {
            ...
            for (let i = 0; i < targetIds.length; i++) {
                ...
                let boundary = points.map(array => utilities.transformWorldToIndex(imageData, array)).map(array => [Math.round(array[0]),Math.round(array[1]),sliceIndex])
                //let fillPoints = this.getScanlineFilledPoints3D(this.cleanAndDensifyPolyline(polyline))
